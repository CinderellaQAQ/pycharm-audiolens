...
> **PyCharm 移植版：**当前分支构建 **AudioLens for PyCharm**。安装方式、已支持功能、SFTP 远端音频、文本路径导航、诊断、隐私和构建说明见 [PyCharm 中文文档](PYCHARM.zh-CN.md)。下方保留上游 VS Code 文档作为功能参考。PyCharm 版支持“本地 PyCharm + 已配置的 SFTP Deployment”，不支持 Remote Development/Gateway 和远端 Ark 文件。
...
| 多通道音频 | Audacity 风格的多通道音轨、每通道波形图/语谱图视图、静音、独奏，以及每通道增益/声道平衡播放。双声道默认将声道 1 设为全左、声道 2 设为全右；单声道和三个及以上声道继续居中下混。每通道可独立调整频率刻度/范围与振幅，纵轴刻度随音轨高度自适应。 |
...
```bash
# 从 https://github.com/SimZhou/vscode-audiolens/releases 下载 audiolens-<version>.vsix
...
- 拖动音轨底部边界可以调整整条音轨的高度；多视图下拖动波形图与频谱图之间的边界，可以调整二者的高度比例。
- 双击任一可拖动边界可以恢复已保存的默认布局；最新布局会在打开其他音频文件时继续使用。
