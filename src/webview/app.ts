...
import {
  ...
  sampleValueScaleFor,
  SourceSampleValueFormat,
  sourceSampleValueFormatOf
} from "../shared/sampleValue";
...
interface AnalysisSettings {
  ...
  amplitudeScaleMode: AmplitudeScaleMode;
  // Stored internally in normalized decoder units; Sample mode converts these to/from source PCM values at the UI boundary.
  amplitudeMin: number;
  amplitudeMax: number;
  timeZoom: number;
  timeOffset: number;
...
export class AudioLensApp {
  ...
  private sourceSampleValueFormat: SourceSampleValueFormat | undefined;
  ...
  private selectionAnalysisVisible = false;
  ...
  public async handleMessage(message: ExtensionMessage): Promise<void> {
    switch (message.type) {
      case "bootstrap":
        ...
        this.settings.amplitudeScaleMode = normalizeAmplitudeScaleMode(message.config.analysis.amplitudeScaleMode);
  ...
  private clearDecodedAudio(): void {
    ...
    this.sourceSampleValueFormat = undefined;
  ...
  private async load(metadata: AudioFileMetadata): Promise<void> {
    ...
    this.syncAmplitudeRangeControls();
  ...
  private async loadEncoded(fileName: string): Promise<void> {
    ...
    try {
      ...
      this.sourceSampleValueFormat = sourceSampleValueFormatOf(facts);
  ...
  private async loadEncodedViaFfmpeg(fileName: string): Promise<void> {
    ...
    try {
      ...
      this.sourceSampleValueFormat = sourceSampleValueFormatOf(
        this.audioBytes ? readAudioFileFacts(this.audioBytes, fileName) : {}
      );
  ...
  private loadWavePcmBytes(bytes: Uint8Array, audioContext: BaseAudioContext): boolean {
    ...
    this.sourceSampleValueFormat = sourceSampleValueFormatOf(parsed.format);
    return true;
  }
  ...
  private bindUi(): void {
    ...
    this.elements.selectionAnalysisClose.addEventListener("click", () => {
      this.hideSelectionAnalysis();
    });
    ...
    this.elements.amplitudeScaleMode.addEventListener("change", () => {
      this.settings.amplitudeScaleMode = normalizeAmplitudeScaleMode(this.elements.amplitudeScaleMode.value);
    ...
    const onAmplitudeRange = () => {
      const sampleLo = Number(this.elements.amplitudeMinInput.value);
      const sampleHi = Number(this.elements.amplitudeMaxInput.value);
      if (Number.isFinite(sampleLo) && Number.isFinite(sampleHi) && sampleHi > sampleLo) {
        const scale = this.sampleValueScale();
        this.settings.amplitudeMin = normalizedFromSampleValue(sampleLo, scale);
        this.settings.amplitudeMax = normalizedFromSampleValue(sampleHi, scale);
        this.settings.amplitudeScaleMode = "sample";
        this.elements.amplitudeScaleMode.value = "sample";
        this.syncAmplitudeRangeControls();
        this.updateDynamicSettingHelp();
        this.savePreferencesSoon();
        this.updateResetViewButtonState();
  ...
  private handleEscape(): void {
    ...
    if (this.selectionAnalysisVisible) {
      this.hideSelectionAnalysis();
      return;
    }
    if (this.selection) {
      this.selection = undefined;
      this.selectionAnalysisVisible = false;
      this.selectionPlaybackEnd = undefined;
      this.updateSelectionAnalysis();
      this.redrawVisuals();
      return;
  ...
  private bindAnalysisTooltips(): void {
    document.querySelectorAll<HTMLElement>(".analysisHelp, .metricHelp, .settingHelp").forEach((trigger) => {
  ...
  private syncControls(): void {
    ...
    this.elements.amplitudeScaleMode.value = this.settings.amplitudeScaleMode;
    const usesManualSampleRange = this.settings.amplitudeScaleMode === "sample";
    this.elements.amplitudeMinInput.disabled = !usesManualSampleRange;
    this.elements.amplitudeMaxInput.disabled = !usesManualSampleRange;
    this.syncAmplitudeRangeControls();
  ...
  private sampleValueScale(): SampleValueScale {
    return sampleValueScaleFor(this.sourceSampleValueFormat);
  }

  private syncAmplitudeRangeControls(): void {
    const scale = this.sampleValueScale();
    const minValue = sampleValueFromNormalized(this.settings.amplitudeMin, scale);
    const maxValue = sampleValueFromNormalized(this.settings.amplitudeMax, scale);
    this.elements.amplitudeMinInput.min = String(scale.min);
    this.elements.amplitudeMinInput.max = String(scale.max);
    this.elements.amplitudeMaxInput.min = String(scale.min);
    this.elements.amplitudeMaxInput.max = String(scale.max);
    this.elements.amplitudeMinInput.step = scale.integer ? "1" : "0.1";
    this.elements.amplitudeMaxInput.step = scale.integer ? "1" : "0.1";
    this.elements.amplitudeMinInput.value = formatSampleValueInput(minValue, scale);
    this.elements.amplitudeMaxInput.value = formatSampleValueInput(maxValue, scale);
  }

  private updateDynamicSettingHelp(): void {
    const windowHelpKeys: Record<WindowFunction, keyof LocaleMessages> = {
      rectangular: "windowRectangularHelp",
      bartlett: "windowBartlettHelp",
      hamming: "windowHammingHelp",
      hann: "windowHannHelp",
      blackman: "windowBlackmanHelp",
      blackmanHarris: "windowBlackmanHarrisHelp",
      welch: "windowWelchHelp",
      gaussian25: "windowGaussian25Help",
      gaussian35: "windowGaussian35Help",
      gaussian45: "windowGaussian45Help"
    };
    const amplitudeHelpKeys: Record<AmplitudeScaleMode, keyof LocaleMessages> = {
      adaptive: "amplitudeScaleAdaptiveHelp",
      sample: "amplitudeScaleSampleHelp",
      decibel: "amplitudeScaleDecibelHelp"
    };
    this.elements.windowFunctionHelp.dataset.tooltip = this.messages[windowHelpKeys[this.settings.windowFunction]];
    this.elements.amplitudeScaleHelp.dataset.tooltip = this.messages[amplitudeHelpKeys[this.settings.amplitudeScaleMode]];
  }
  ...
  private applyPreferences(preferences: AudioLensPreferences): void {
    ...
    if (preferences.amplitudeScaleMode !== undefined) {
      this.settings.amplitudeScaleMode = normalizeAmplitudeScaleMode(preferences.amplitudeScaleMode);
    } else if (preferences.amplitudeAuto !== undefined) {
      this.settings.amplitudeScaleMode = preferences.amplitudeAuto ? "adaptive" : "sample";
    }
  ...
  private collectPreferences(): AudioLensPreferences {
    return {
      ...
      amplitudeScaleMode: this.settings.amplitudeScaleMode,
      amplitudeAuto: this.settings.amplitudeScaleMode === "adaptive",
  ...
  private resetView(): void {
    ...
    this.settings.amplitudeScaleMode = "adaptive";
    for (const view of this.trackViews) {
      view.ampRangeOverride = undefined;
    }
    this.selection = undefined;
  ...
  private async applyPcmFormat(format: PcmFormat, statusElement = this.elements.pcmStatus): Promise<boolean> {
    ...
    this.track = this.audioBuffer.sampleRate === nativeTrack.sampleRate
      ...
      : nativeTrack;
    this.sourceSampleRate = decoded.sampleRate;
    this.sourceSampleValueFormat = sourceSampleValueFormatOf(format);
  ...
  private drawChannelWaveform(canvas: HTMLCanvasElement, channel: number): void {
    ...
    const yOf = (v: number) => this.settings.amplitudeScaleMode === "decibel"
      ? waveformDbY(v, rect, WAVEFORM_DB_FLOOR)
      : clamp(rect.bottom - ((v - channelRange.min) / span) * rect.height, rect.top, rect.bottom);
  ...
  private handleWheel(event: WheelEvent, canvas: HTMLCanvasElement): void {
    ...
    if (this.canvasClientX(canvas, event.clientX) < plotRect.left) {
      ...
      if (view && (timeZoomModifier || trackpadPinchZoom)) {
        if (isSpec) {
          ...
          this.redrawVisuals();
          this.scheduleAnalyze();
        } else if (isWave && this.settings.amplitudeScaleMode !== "decibel") {
    ...
    if (event.altKey && canvas.classList.contains("trackWaveform") && this.settings.amplitudeScaleMode !== "decibel") {
      event.preventDefault();
      const channel = Number(canvas.dataset.channel ?? 0);
      const view = this.trackViews.find((v) => v.channel === channel);
      if (view) {
  ...
  private handleSelectionContextMenuClick(event: Event): void {
    ...
    this.hideSelectionContextMenu();
    if (action === "show-selection-analysis") {
      this.selectionAnalysisVisible = true;
      this.updateSelectionAnalysis();
      return;
    }
  ...
  private updateSelectionAnalysis(): void {
    if (!this.hasAudio() || !this.selection) {
      ...
      this.cancelSelectionSpectrumAnalysis();
      this.elements.selectionAnalysisPane.hidden = true;
    ...
    this.elements.selectionAnalysisPane.hidden = false;
  ...
  private hideSelectionAnalysis(): void {
    this.selectionAnalysisVisible = false;
    this.selectionDataRequestSeq += 1;
    this.cancelSelectionSpectrumAnalysis();
    this.elements.selectionAnalysisPane.hidden = true;
  ...
  private effectiveAmplitudeRange(channel: number): { min: number; max: number } {
    ...
    if (this.settings.amplitudeScaleMode === "adaptive") {
      return this.autoAmplitudeRange(channel);
    }
    if (this.settings.amplitudeScaleMode === "decibel") {
      return { min: -1, max: 1 };
    }
    const scale = this.sampleValueScale();
    const min = normalizedFromSampleValue(sampleValueFromNormalized(this.settings.amplitudeMin, scale), scale);
    const max = normalizedFromSampleValue(sampleValueFromNormalized(this.settings.amplitudeMax, scale), scale);
    return max > min
      ? { min, max }
      : { min: normalizedFromSampleValue(scale.min, scale), max: normalizedFromSampleValue(scale.max, scale) };
  }
  ...
  private drawWaveformAxis(context: CanvasRenderingContext2D, rect: PlotRect, channel: number): void {
    ...
    for (let index = 0; index <= intervals; index += 1) {
      ...
      if (index === 0) {
        ...
        context.fillText(this.formatWaveformAxisTick(value, ratio), rect.left - devicePx(6), rect.top + devicePx(2));
      } else if (index === intervals) {
        context.textBaseline = "bottom";
        context.fillText(this.formatWaveformAxisTick(value, ratio), rect.left - devicePx(6), rect.bottom - devicePx(2));
      } else {
        context.textBaseline = "middle";
        context.fillText(this.formatWaveformAxisTick(value, ratio), rect.left - devicePx(6), y);
      }
    }
    context.restore();
  }

  private formatWaveformAxisTick(value: number, verticalRatio: number): string {
    if (this.settings.amplitudeScaleMode === "sample") {
      const scale = this.sampleValueScale();
      return formatSampleValueAxis(sampleValueFromNormalized(value, scale), scale);
    }
    if (this.settings.amplitudeScaleMode !== "decibel") {
      return formatAmplitudeAxis(value);
    }
    const distanceFromCenter = Math.abs(1 - verticalRatio * 2);
    const db = WAVEFORM_DB_FLOOR + distanceFromCenter * -WAVEFORM_DB_FLOOR;
...
function waveformDbY(value: number, rect: PlotRect, floorDb: number): number {
  if (!Number.isFinite(value) || value === 0) {
    return rect.top + rect.height / 2;
  }
  const db = clamp(amplitudeToDb(Math.abs(value)), floorDb, 0);
  const normalizedMagnitude = (db - floorDb) / -floorDb;
  const signedPosition = Math.sign(value) * normalizedMagnitude;
  return clamp(rect.top + ((1 - signedPosition) / 2) * rect.height, rect.top, rect.bottom);
}

function formatSampleValueInput(value: number, scale: SampleValueScale): string {
  return scale.integer ? String(Math.round(value)) : String(Math.round(value * 1_000_000) / 1_000_000);
}

function formatSampleValueAxis(value: number, scale: SampleValueScale): string {
  if (!scale.integer) {
    return formatAmplitudeAxis(value);
  }
  ...
  if (magnitude < 1_000_000_000) {
    return `${trimAxisNumber(rounded / 1_000_000)}M`;
  }
  return `${trimAxisNumber(rounded / 1_000_000_000)}G`;
}

function trimAxisNumber(value: number): string {
  return value.toFixed(Math.abs(value) >= 100 ? 0 : Math.abs(value) >= 10 ? 1 : 2).replace(/\.0+$/, "");
}

function normalizeAmplitudeScaleMode(value: unknown): AmplitudeScaleMode {
  return value === "sample" || value === "decibel" ? value : "adaptive";
}
