...
export const messages = {
  ...
  windowSize: "窗口大小",
  windowSizeHelp: "每个 FFT 分析帧包含的采样点数。数值越大，频率细节越清楚，但时间细节和计算速度会下降。",
  windowType: "窗口类型",
  ...
  windowRectangularHelp: "矩形窗：不衰减帧内采样，主瓣较窄；如果信号在帧内不是完整周期，频谱泄漏和旁瓣会比较明显。",
  windowBartlettHelp: "Bartlett 三角窗：从中心向两端线性衰减，相比矩形窗可减小边界突变和频谱泄漏。",
  windowHammingHelp: "汉明窗：平滑衰减分析帧两端，在频率分辨率与泄漏抑制之间较均衡，是语音频谱分析中的常用选择。",
  windowHannHelp: "Hann（汉宁）窗：两端衰减到零，具有可靠的通用频谱泄漏抑制效果。",
  windowBlackmanHelp: "Blackman 窗：进一步降低旁瓣，便于观察较弱频率成分，但主瓣会变宽。",
  windowBlackmanHarrisHelp: "Blackman-Harris 窗：旁瓣抑制很强、动态范围较好，但会牺牲更多频率分辨率。",
  windowWelchHelp: "Welch 窗：采用抛物线形衰减，在主瓣宽度和泄漏抑制之间折中。",
  windowGaussian25Help: "高斯窗 α=2.5：衰减较温和，在时间集中度和频率集中度之间保持折中。",
  windowGaussian35Help: "高斯窗 α=3.5：比 α=2.5 的边缘衰减更强，可进一步降低边界影响。",
  windowGaussian45Help: "高斯窗 α=4.5：这里最强的高斯衰减，旁瓣更低，但有效时间范围也更短。",
  zeroPaddingFactor: "零填充因子",
  zeroPaddingFactorHelp: "在 FFT 前向每帧尾部补零，使频谱的频率采样网格更密、显示插值更平滑；它不会增加真实频率分辨率。",
  frequencyScale: "频率刻度",
  frequencyScaleHelp: "控制频率轴的纵向间距：线性使用等 Hz 间隔；对数、Mel、Bark 和 ERB 会突出听觉上更重要的低频细节。",
  frequencyRange: "频率范围（仅显示）",
  minFrequencyHz: "最小频率 (Hz)",
  minFrequencyHzHelp: "频谱图显示的最低频率。更低的频率只是被隐藏，不会从音频中删除。",
  maxFrequencyHz: "最大频率 (Hz)",
  maxFrequencyHzHelp: "频谱图显示的最高频率，不能超过采样率一半的奈奎斯特频率。",
  maxFrequencyNyquist: "最大值跟随 Nyquist",
  maxFrequencyNyquistHelp: "自动把显示上限设为当前采样率的一半，以显示音频可测量的完整频段。",
  spectrogramAppearance: "频谱图外观",
  palette: "调色板",
  paletteHelp: "把频谱能量映射为颜色，只影响显示效果，不会改变 FFT 数值或音频。",
  paletteRose: "颜色 (玫瑰)",
  paletteClassic: "颜色 (经典)",
  paletteGrayscale: "灰度",
  paletteInverseGrayscale: "反相灰度",
  minDb: "最小 dB (亮度)",
  minDbHelp: "低于此 dB 值的频谱能量使用调色板中的最暗颜色；降低它可以显示更弱的成分和噪声。",
  maxDb: "最大 dB (亮度)",
  maxDbHelp: "高于此 dB 值的频谱能量使用调色板中的最亮颜色；它控制显示对比度，不改变播放音量。",
  autoBrightness: "自动亮度",
  autoBrightnessHelp: "根据当前音频估算合适的频谱最小和最大 dB；手动修改 dB 范围会关闭自动亮度。",
  amplitudeRange: "振幅范围（波形）",
  amplitudeScale: "幅值刻度",
  amplitudeScaleAdaptive: "自适应",
  amplitudeScaleSample: "采样值",
  amplitudeScaleDecibel: "分贝（dBFS）",
  amplitudeScaleAdaptiveHelp: "自适应：按每个声道的实测峰值自动调整线性采样值范围，让较小幅值的波形也能充分显示。",
  amplitudeScaleSampleHelp: "采样值：显示音源 PCM 的原始数值，例如 16 位有符号音频为 -32768 到 32767；浮点音频或压缩格式的解码输出仍为 -1 到 +1。可在下方自定义上下限。",
  amplitudeScaleDecibelHelp: "分贝：使用对称的 dBFS 对数波形刻度。0 dBFS 表示数字满幅，低于 -60 dBFS 的部分汇合到中心显示下限。",
  minAmplitude: "最小振幅",
  minAmplitudeHelp: "波形坐标轴的原始采样值下限，仅在“采样值”模式中使用。",
  maxAmplitude: "最大振幅",
  maxAmplitudeHelp: "波形坐标轴的原始采样值上限，仅在“采样值”模式中使用。",
  ...
  sampleRate: "采样率",
  sampleRateHelp: "PCM 每秒包含的采样点数。设置错误会改变播放速度，并使时间和频率测量结果不正确。",
  channels: "通道数",
  channelsHelp: "PCM 数据中交错存储的声道数量，例如单声道为 1，立体声为 2。",
  startOffsetBytes: "偏移(B)",
  startOffsetBytesHelp: "开始读取 PCM 采样前需要跳过的字节数；原始数据带有自定义文件头时使用。",
  bitDepth: "编码",
  bitDepthHelp: "每个 PCM 采样的存储方式，包括有符号/无符号整数或浮点数，以及相应位深。",
  sampleFormat: "格式",
  endianness: "字节序",
  endiannessHelp: "多字节 PCM 采样的字节排列顺序：小端把低有效字节放在前面，大端则放在后面。",
  read: "读取",
  saveDefault: "保存默认",
  cancel: "取消",
  defaultView: "默认视图",
  view: "视图",
  viewHelp: "选择每个声道音轨初始显示波形、频谱图或两者；打开后仍可单独修改每条音轨。",
  viewBoth: "多视图",
  mute: "静音",
  solo: "独奏",
  timeLabel: "时间",
