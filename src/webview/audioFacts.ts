...
export function readAudioFileFacts(bytes: Uint8Array, fileName: string): AudioFileFacts {
  ...
  if (extension === "wav" || extension === "wave") {
    return readWavFacts(bytes);
  }
  if (extension === "flac") {
    return readFlacFacts(bytes);
  }
  return {};
}
...
function readWavFacts(bytes: Uint8Array): AudioFileFacts {
  ...
  while (offset + 8 <= bytes.byteLength) {
    ...
    const chunkEnd = Math.min(bytes.byteLength, dataOffset + size);
    if (id === "fmt " && dataOffset + 16 <= chunkEnd) {
      const sampleRate = view.getUint32(dataOffset + 4, true);
      const containerBits = view.getUint16(dataOffset + 14, true);
      const audioFormat = view.getUint16(dataOffset, true);
      const extensibleFormat = audioFormat === 0xfffe && dataOffset + 26 <= chunkEnd
        ? view.getUint16(dataOffset + 24, true)
        : audioFormat;
      const validBits = audioFormat === 0xfffe && dataOffset + 20 <= chunkEnd
        ? view.getUint16(dataOffset + 18, true)
        : 0;
      const bitDepth = validBits > 0 ? validBits : containerBits;
      const sampleFormat = extensibleFormat === 3
        ? "float"
        : extensibleFormat === 1
          ? (containerBits === 8 ? "unsigned-int" : "signed-int")
          : undefined;
      return {
...
function readFlacFacts(bytes: Uint8Array): AudioFileFacts {
  ...
  const bitDepth = (((bytes[offset + 12] & 0x01) << 4) | (bytes[offset + 13] >> 4)) + 1;
  return {
    sampleRate: sampleRate > 0 ? sampleRate : undefined,
    bitDepth,
